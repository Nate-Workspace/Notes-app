...
function Notes(){
    ...
    function handleChange(event){
        //itteration=event.target.value
        setDisplay(event.target.value)
    }
    
    function addList(){
        setData(prevData=>{
            return [...prevData,`${display}`]
        })
    ...
    const DataElements= Data.map(each=> {
        key++
        return <p key={key} className="notesDisplay">{each}</p>
    })
    return(
        <>
        <div className="Wrapper">
            ...
            <form>
            <textarea className='Form' placeholder="what is your plan?" onChange={handleChange} value={display} />
            </form>
            <button className="addList" onClick={addList}>Add list</button>
            {DataElements}
        </div> 
        </>
    )
